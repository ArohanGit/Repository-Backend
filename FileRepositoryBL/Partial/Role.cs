...
namespace FileRepository.BusinessObjects
{
    public partial class Role : Entity<Role>
    {
        ...
        public Role(bool defaults) : base(defaults) { }
        public Role()
        {
            Settings();
        ...
        public override void OnAudit(string eventType, Entity<Role> oldEntity, List<AuditLog> log)
        {
        ...
        public List<Role> SaveList(List<Role> oRoleList)
        {
            try
            {
                if (oRoleList == null || oRoleList.Count <= 0) return oRoleList;
                foreach (Role oRole in oRoleList)
                {
                    oRole.Save();
                }
                return oRoleList;
